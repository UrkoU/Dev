...
function showSongs() {
    ...
    for(let i = 0; i < songs.length; i++)divSongs.innerHTML += `
...
const syncVolume = (volume)=>{
    if (volume > 0.5) volumeBtn.textContent = "volume_up";
    else if (volume < 0.5 && volume > 0) volumeBtn.textContent = "volume_down";
    else if (volume === 0) volumeBtn.textContent = "volume_mute";
};
